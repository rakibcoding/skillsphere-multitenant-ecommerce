...
export const checkoutRouter = createTRPCRouter({
  verify: protectProcedure.mutation(async ({ ctx }) => {
    const user = await ctx.payload.findByID({
      collection: "users",
      id: ctx.session.user.id,
      depth: 0, // user.tenant[0].tenant is going to string [tenant ID]
    });
  ...
  purchase: protectProcedure
    ...
    .mutation(async ({ ctx, input }) => {
        ...
        products.docs.map((product) => ({
          ...
          price_data: {
            ...
            product_data: {
              name: product.name,
              metadata: {
                stripeAccId: tenant.stripeAccId,
                id: product.id,
                name: product.name,
                price: product.price,
              } as ProductMetadata,
      ...
      const checkout = await stripe.checkout.sessions.create(
        ...
        { stripeAccount: tenant.stripeAccId }
      );
      if (!checkout.url) {
        throw new TRPCError({
